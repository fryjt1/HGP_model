...
# Simulate Data
data = Simulate.Data.Probit(n=n,p=p,R=R,phi=rep(35,R),coords=coords,beta.mean=0,sigma2.beta=sigma2.beta.true,
                            phi.beta=phi.beta.true,centroids=centroids.true,var.select=TRUE,theta=theta.true,pj=.5)
...
I = 20000
y.store = matrix(NA,I,n); y.store[1,] = z.all*2-1
...
for(r in 1:R)
{
  ...
  S.list.curr[[r]] = (exp(-D[ind,ind]/phi.curr[r]) + diag(1e-8,length(ind)))
...
for(i in 2:I)
{
  y.curr = Latent.Draw(X.all=X.all,y.curr=y.curr,z.all=z.all,clusters=clusters.curr,beta.mat=beta.curr,
  ...
  draw = GP.Phi.Sample.Probit(X.all=X.all,y.all=y.curr,clusters=clusters.curr,D=D,step=1,phi.UB=2,mu=mu.curr,
  ...
  draw = Sample.Mu(y.all=y.curr,X.all=X.all,S.inv.list=S.inv.list.curr,K=K.curr,
                   clusters=clusters.curr,beta=beta.curr,mu.curr=mu.curr)
  mu.curr = draw
  
  draw = SSVS.Gibbs(y.all=y.curr,X.all=X.all,clusters=clusters.curr,S.inv.list=S.inv.list.curr,K=K.curr,
                    mu=mu.curr,beta.curr=beta.curr,gamma.curr=gamma.curr,probs=probs)
  gamma.curr = draw$gamma.draw
  beta.curr = draw$beta.draw
  
  draw = Cluster.Assign.CH(X.all=X.all,y.all=y.curr,clusters.curr=clusters.curr,beta=beta.curr,mu=mu.curr,
