...
Simulate.Data.Probit = function(n,p,R,phi=rep(1e-10,R),beta.mean=0,sigma2.beta,phi.beta,coords,centroids,var.select=FALSE,theta=0,pj=.5)
{
  clusters = apply(distance(coords,centroids),1,which.min)
  W = 1/sqrt(distance(centroids)); diag(W) = 0
  ...
  {
    ind = which(clusters==r)
    S.list[[r]] = (exp(-D[ind,ind]/phi[r]) + diag(1e-8,length(ind)))
    S.inv.list[[r]] = solve(S.list[[r]])
    y.all[ind,] = mu[,r] + X.all[ind,,drop=F]%*%beta[,r,drop=F] + 
      c(rmvnorm(1,rep(0,length(ind)),S.list[[r]]))
  }
  z.all = matrix(rep(1,n),ncol=1)
  z.all[y.all<0,] = 0
  
  return(list(z.all=z.all,y.all=y.all,X.all=X.all,coords=coords,centroids=centroids,clusters=clusters,sigma2.beta=sigma2.beta,phi.beta=phi.beta,W=W,gamma=gamma,pj=pj,phi=phi,beta=beta,mu=mu,K=K,D=D,S.list=S.list,S.inv.list=S.inv.list,theta=theta))
}
...
GP.Phi.Sample.Probit = function(X.all,y.all,clusters,D,step,phi.UB,mu,beta,phi.curr,
                                S.list.curr,S.inv.list.curr)
{
  R = length(phi.curr)
  phi.draw = rep(NA,R)
  S.list.draw = list()
  ...
  for(r in 1:R)
  {
    ...
    denom = ( -.5*determinant(S.curr,logarithm=TRUE)$modulus-.5*t(y.curl)%*%S.curr.inv%*%(y.curl)
              + log(Phi.prior.Probit(D[ind,ind],phi=phi.c))
...
Phi.prior.Probit = function(D,phi)
{
  S = exp(-D/phi) + diag(1e-8,nrow(D))
  U = (S*D/phi^2)%*%solve(S)
  prior = ( sum(diag(U%*%U)) - sum(diag(U))^2/n )^.5
  return(prior)
}
...
Sample.phi.beta = function(mu,beta,gamma,phi.beta.curr,step,K.inv.curr,W,sigma2.beta)
{
  R = ncol(beta)
  p = nrow(beta)
  mu = matrix(mu,nrow=1)
...
Sample.theta = function(theta.curr,gamma,W,a,b,theta.UB,step.size)
{
  ...
  log.denom = ( sum(log(p.curr[gamma==1]))+sum(log((1-p.curr)[gamma==0])) - (a-1)*log(max(theta.curr,1e-8)) - b*theta.curr
...
Find.Nearest.Cluster = function(CHs,R,coords.new)
{
  dists = rep(NA,R)
  for(r in 1:R)
  {
    d = dist2Line(coords.new,CHs[[r]])
    dists[r] = d[,1]
  }
  return( which.min(dists))
}

Predict.ynew = function(x.new,coords.new,coords,clusters,S.inv.list,beta,mu,X,y,phi,CHs)
{
  R = ncol(beta)
  assign = Find.Nearest.Cluster(CHs=CHs,R=R,coords.new=coords.new)
  ind = which(clusters==assign)
  
  S12 = matrix(exp(-distance(coords[ind,],matrix(coords.new,nrow=1))/phi[assign]),nrow=1)
  S22.inv = S.inv.list[[assign]]
  S12.S22.inv = S12%*%S22.inv
  V = max(1e-8,1 - S12.S22.inv%*%t(S12))
  E = mu[assign] + matrix(x.new,nrow=1)%*%beta[,assign,drop=F] + S12.S22.inv%*%(y[ind,]-mu[assign]-X[ind,]%*%beta[,assign,drop=F])
  y.pred = rnorm(1,E,sqrt(V))
  return(y.pred)
}
