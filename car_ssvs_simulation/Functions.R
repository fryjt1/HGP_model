                         ...
                         beta.mean=0,sigma2.beta,phi.beta,centroids,var.select=FALSE,theta=0,pj=.5)
{
  ...
  if(var.select==TRUE)
  {
    ...
    gamma = foreach(i=1:p,.combine='rbind',.export=c("Sim.Ising")) %dopar% Sim.Ising(n=R,theta=theta,W=W,B=1000)
    stopCluster(cl)
    }
  }else{
    gamma = matrix(1,p,R)
...
# Sample.phi.beta = function(mu,beta,gamma,phi.beta.curr,phi.UB,step,K.curr,D.centroids,tau2.beta,g.beta)
# {
#   R = ncol(beta)
#   p = nrow(beta)
#   mu = matrix(mu,nrow=1)
#   
#   beta.sub = beta[which(rowSums(gamma)>0),]
#   gamma.sub = gamma[which(rowSums(gamma)>0),]
#   beta.j = lapply(split(beta.sub,seq(nrow(beta.sub))),function(x) x[x!=0])
#   
#   phi.beta.prop = rtruncnorm(1,a=0,b=phi.UB,mean=phi.beta.curr,sd=step)
#   K.prop = tau2.beta*(exp(-D.centroids/phi.beta.prop) + diag(g.beta,R))
#   
#   K.j.prop = lapply(split(gamma.sub,seq(nrow(gamma.sub))),function(x) K.prop[which(x==1),which(x==1)])
#   K.j.curr = lapply(split(gamma.sub,seq(nrow(gamma.sub))),function(x) K.curr[which(x==1),which(x==1)])
#   
#   log.det.prop = determinant(K.prop,logarithm=TRUE)$modulus + sum(sapply(K.j.prop,function(x) determinant(as.matrix(x),logarithm=TRUE)$modulus))
#   log.det.curr = determinant(K.curr,logarithm=TRUE)$modulus + sum(sapply(K.j.curr,function(x) determinant(as.matrix(x),logarithm=TRUE)$modulus))
#   
#   SS.prop = ( mu%*%solve(K.prop)%*%t(mu) +
#                 sum(unlist(Map("%*%",Map("%*%",beta.j,lapply(K.j.prop,function(x) solve(x))),beta.j))) )
#   SS.curr = ( mu%*%solve(K.curr)%*%t(mu) +
#                 sum(unlist(Map("%*%",Map("%*%",beta.j,lapply(K.j.curr,function(x) solve(x))),beta.j))) )
#   
#   numer = ( -.5*log.det.prop - .5*SS.prop
#             + Calc.phi_g.prior(D=D.centroids,S=K.prop,S.inv=solve(K.prop),tau2=tau2.beta,g=g.beta,phi=phi.beta.prop)
#             - log(pnorm(phi.UB,mean=phi.beta.prop,sd=step)-pnorm(0,mean=phi.beta.prop,sd=step)) )
#   denom = ( -.5*log.det.curr - .5*SS.curr
#             + Calc.phi_g.prior(D=D.centroids,S=K.curr,S.inv=solve(K.curr),tau2=tau2.beta,g=g.beta,phi=phi.beta.curr)
#             - log(pnorm(phi.UB,mean=phi.beta.curr,sd=step)-pnorm(0,mean=phi.beta.curr,sd=step))  )
#   
#   if(log(runif(1))<numer-denom)
#   {
...
#     K.draw = K.curr
#   }
#   return(list(phi.beta.draw = phi.beta.draw, K.draw = K.draw))
# }
# 
# Sample.g.beta = function(mu,beta,gamma,phi.beta,g.UB,step,K.curr,D.centroids,tau2.beta,g.beta.curr)
# {
#   R = ncol(beta)
#   p = nrow(beta)
#   mu = matrix(mu,nrow=1)
...
#   beta.j = lapply(split(beta.sub,seq(nrow(beta.sub))),function(x) x[x!=0])
#   
#   g.beta.prop = rtruncnorm(1,a=0,b=g.UB,mean=g.beta.curr,sd=step)
#   K.prop = tau2.beta*(exp(-D.centroids/phi.beta) + diag(g.beta.prop,R))
#   
#   K.j.prop = lapply(split(gamma.sub,seq(nrow(gamma.sub))),function(x) K.prop[which(x==1),which(x==1)])
#   K.j.curr = lapply(split(gamma.sub,seq(nrow(gamma.sub))),function(x) K.curr[which(x==1),which(x==1)])
#   
...
#   
#   numer = ( -.5*log.det.prop - .5*SS.prop
#             + Calc.phi_g.prior(D=D.centroids,S=K.prop,S.inv=solve(K.prop),tau2=tau2.beta,g=g.beta.prop,phi=phi.beta)
#             - log(pnorm(g.UB,mean=g.beta.prop,sd=step)-pnorm(0,mean=g.beta.prop,sd=step)) )
#   denom = ( -.5*log.det.curr - .5*SS.curr
#             + Calc.phi_g.prior(D=D.centroids,S=K.curr,S.inv=solve(K.curr),tau2=tau2.beta,g=g.beta.curr,phi=phi.beta)
#             - log(pnorm(g.UB,mean=g.beta.curr,sd=step)-pnorm(0,mean=g.beta.curr,sd=step))  )
#   
#   if(log(runif(1))<numer-denom)
#   {
#     g.beta.draw = g.beta.prop
#     K.draw = K.prop
#   }else
#   {
#     g.beta.draw = g.beta.curr
#     K.draw = K.curr
#   }
#   return(list(g.beta.draw = g.beta.draw, K.draw = K.draw))
# }
# 
# # mu = mu.true; beta = beta.true; gamma = gamma.true; K.curr = K.true; tau2.beta.curr = 1
# Sample.tau2.beta = function(mu,beta,gamma,K.curr,tau2.beta.curr)
# {
#   R = ncol(beta)
#   mu = matrix(mu,nrow=1)
#   
...
#   gamma.sub = gamma[which(rowSums(gamma)>0),]
#   beta.j = lapply(split(beta.sub,seq(nrow(beta.sub))),function(x) x[x!=0])
#   K.curl = K.curr/tau2.beta.curr
#   K.j.curl = lapply(split(gamma.sub,seq(nrow(gamma.sub))),function(x) K.curl[which(x==1),which(x==1)])
# 
#   SS = ( mu%*%solve(K.curl)%*%t(mu) +
#                 sum(unlist(Map("%*%",Map("%*%",beta.j,lapply(K.j.curl,function(x) solve(x))),beta.j))) )
#   a.star = .5*(R + sum(gamma))
#   b.star = .5*SS
#   tau2.beta.draw = 1/rgamma(1,shape=a.star,rate=b.star)
#   K.draw = K.curl*tau2.beta.draw
#   
#   return(list(tau2.beta.draw=tau2.beta.draw,K.draw=K.draw))
# }

# mu = mu.true; beta = beta.true; gamma = gamma.true; phi.beta.curr = .1; K.inv.curr = solve(K.true)
# D.centroids = D.centroids; sigma2.beta = 1; step = .01
...
# Negative LL that takes phi.beta and g.beta separately
nLL.beta = function(phi.beta,g.beta,mu,gamma,beta,D.centroids)
{
  mu = matrix(mu,nrow=1)
  R = ncol(mu)
  beta = matrix(beta,ncol=R)
  gamma = matrix(gamma,ncol=R)
  p = nrow(beta)

  beta.sub = beta[which(rowSums(gamma)>0),,drop=F]
  gamma.sub = gamma[which(rowSums(gamma)>0),,drop=F]
  beta.j = lapply(split(beta.sub,seq(nrow(beta.sub))),function(x) x[x!=0])
  C = exp(-D.centroids/phi.beta) + diag(g.beta,R)
  Cj = lapply(split(gamma.sub,seq(nrow(gamma.sub))),function(x) C[which(x==1),which(x==1)])
  
  log.det = determinant(C,logarithm=TRUE)$modulus + sum(sapply(Cj,function(x) determinant(as.matrix(x),logarithm=TRUE)$modulus))
  SS = ( mu%*%solve(C)%*%t(mu) +
         ifelse(sum(gamma>0),sum(unlist(Map("%*%",Map("%*%",beta.j,lapply(Cj,function(x) solve(x))),beta.j))),0) )
  
  g.args = garg(list(mle = TRUE),y=c(beta.curr[gamma.curr==1]))
  nLL = -( -.5*(R+sum(gamma))*log(SS) - .5*log.det - (g.args$ab[1]-1)*log(g.beta) - g.args$ab[2]*g.beta )
  return(nLL)
}

# Negative LL that takes phi and g jointly, for joint optimization
nLL.beta2 = function(pars,mu,gamma,beta,D.centroids,centroids)
{
  phi.beta = pars[1]; g.beta = pars[2]
  mu = matrix(mu,nrow=1)
  R = ncol(mu)
  beta = matrix(beta,ncol=R)
  gamma = matrix(gamma,ncol=R)
  ...
  g.args = garg(list(mle = TRUE),y=c(mu,c(beta[gamma==1])))
  d.args = darg(list(mle = TRUE), centroids) 

  nLL = -( -.5*(R+sum(gamma))*log(SS) - .5*log.det + dhalfcauchy(phi.beta,scale=1,log=TRUE) + 
             dhalfcauchy(g.beta,scale=1,log=TRUE) )
  return(nLL)
}

# Averages over negative LL for Empirical Bayes for phi and g separately
avg.nLL.beta = function(phi.beta,g.beta,mu.mat,gamma.array,beta.array,D.centroids,centroids)
{
  niter = nrow(mu.mat)
  nLL = rep(NA,niter)
  R = ncol(mu.mat)
  ...
  for(i in 1:niter)
  {
    ...
    nLL[i] = -( -.5*(R+sum(gamma))*log(SS) - .5*log.det )
  }
  
  return(mean(nLL))
}

# Averages over negative LL for Empirical Bayes
avg.nLL.beta2 = function(pars,mu.mat,gamma.array,beta.array,D.centroids,centroids)
{
  niter = nrow(mu.mat)
  nLL = rep(NA,niter)
  phi.beta = pars[1]; g.beta = pars[2]
  ...
  for(i in 1:niter)
  {
    ...
    nLL[i] = -( -.5*(R+sum(gamma))*log(SS) - .5*log.det + dhalfcauchy(phi.beta,scale=.5,log=TRUE) + 
                  dhalfcauchy(g.beta,scale=.5,log=TRUE) )
  }

  return(mean(nLL))
}

# Updates tau2.beta using g and phi (Concentrated likelihood)
Update.tau2.beta = function(phi.beta,g.beta,D.centroids,mu,beta,gamma)
{
  mu = matrix(mu,nrow=1)
  R = ncol(mu)
  beta = matrix(beta,ncol=R)
  ...
  return(as.numeric(SS/(R+sum(gamma))))
}
